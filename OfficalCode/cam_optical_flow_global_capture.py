...
def receive_data():
    ...
    while True:
        if ser.in_waiting > 0:
            ...
            try:
                config = -(float(read_data)*7/100) #Relative error = 7%
    ...
    while True:
        if update_task:
            ...
            if not ret:
                ...
                break
            T1Cam = time.time()
            fps_Cam = 0.9 * fps_Cam + 0.1 / (T1Cam - T0Cam)
            T0Cam = T1Cam
            #update_task = False
