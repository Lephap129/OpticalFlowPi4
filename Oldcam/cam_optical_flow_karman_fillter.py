...
def kalman_filter(z,x):
    ...
    x = x_pred + K.dot(y)
    P = (np.eye(len(K)) - K.dot(H)).dot(P_pred)
...
while True:
    ...
    if len(good_new) > 0:
        ...
        X = np.array([global_x, global_y, dx_cm, dy_cm])
        ...
        # Init matrix
        # F is state matrix for union movement
        F = np.array([[1, 0, dt, 0],
                    [0, 1, 0, dt],
                    [0, 0, 1, 0],
                    [0, 0, 0, 1]])
        # H matrix for mesurement of position
        H = np.array([[1, 0, 0, 0],
                    [0, 1, 0, 0]])
        Q = np.eye(4) * 0.01  # System Noise depend on unit of float
        R = np.eye(2) * cm_per_pxl_w  # Measurement noise depend on the unit of pixel in cm
        P = np.eye(4)  # Hiệp phương sai ban đầu
