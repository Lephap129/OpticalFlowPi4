...
while True:
    ...
    if len(good_new) > 0:
        ...
        # if abs(dx_pixels * cm_per_pxl_w) > 0.05*cm_per_pxl_w: dx_cm = sum(list_dx_cm)/(len(list_dx_cm)+1) + (1/(len(list_dx_cm)+1)) * dx_pixels * cm_per_pxl_w
        # else: dx_cm = 0
        # if abs(dy_pixels * cm_per_pxl_h) > 0.05*cm_per_pxl_h: dy_cm = sum(list_dy_cm)/(len(list_dy_cm)+1) + (1/(len(list_dy_cm))) * dy_pixels * cm_per_pxl_h
        # else: dy_cm = 0
